...
class SignerED25519(Signer):
    ...
    _address_versions = {
        SignerSubType.MAINNET_REGULAR: b"\x03\xb8\x6c\xf3",
    ...
    def __init__(
        self,
        private_key: Union[bytes, str] = b"",
        public_key: Union[bytes, str] = b"",
        address: str = "",
        compressed: bool = False,
        subtype: SignerSubType = SignerSubType.MAINNET_REGULAR,
    ):
        super().__init__(
            private_key, public_key, address, compressed=compressed, subtype=subtype
        )
        self._key = None
    ...
    def from_seed(
        self, seed: str = "", subtype: SignerSubType = SignerSubType.MAINNET_REGULAR
    ):
    ...
    def address(self) -> str:
        ...
        base = self.address_version_for_subtype(self._subtype) + bytes.fromhex(
            self._public_key
        )  # raw content
    ...
    def verify_signature(
        ...
        if address != address_rebuild:
            raise ValueError(
                f"Attempt to spend from a wrong address {address} instead of {address_rebuild}"
            )

    @classmethod
    def verify_bis_signature(
        cls, signature: str, public_key: str, buffer: bytes, address: str = ""
    ) -> None:
        ...
        cls.verify_signature(
            b64decode(signature), b64decode(public_key), buffer, address
        )
