...
class SignerECDSA(Signer):
    ...
    _address_versions = {
        SignerSubType.MAINNET_REGULAR: b"\x4f\x54\x5b",
    ...
    def identifier(self):
        ...
        return hashlib.new(
            "ripemd160", sha256(bytes.fromhex(self._public_key)).digest()
        ).digest()

    def address(self):
        """Returns properly serialized address from pubkey as per btc standards"""
        vh160 = (
            self.address_version_for_subtype(self._subtype) + self.identifier()
        )  # raw content
    ...
    def public_key_to_address(
        ...
        if type(public_key) == str:
            identifier = hashlib.new(
                "ripemd160", sha256(bytes.fromhex(public_key)).digest()
            ).digest()
        else:
